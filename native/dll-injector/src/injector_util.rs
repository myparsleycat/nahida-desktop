...
use windows::Win32::System::Threading::{
  ...
  PROCESS_CREATE_THREAD, PROCESS_QUERY_LIMITED_INFORMATION, PROCESS_VM_OPERATION, PROCESS_VM_READ,
  PROCESS_VM_WRITE,
};
...
impl Injector {
  pub fn attach(pid: u32) -> Result<Self> {
    unsafe {
      ...
      let load_library = GetProcAddress(kernel32, PCSTR::from_raw(b"LoadLibraryW\0".as_ptr()));
      let get_last_error = GetProcAddress(kernel32, PCSTR::from_raw(b"GetLastError\0".as_ptr()));
