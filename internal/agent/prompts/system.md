...
Before acting, inspect the Skills catalog and call `load_skill` for every skill whose description matches the request. Loaded skill instructions are mandatory. Load `mod-diagnosis` before diagnosing or repairing a broken or conflicting mod. Intentional content creation, Blender shaping, and source-model conversion should use their matching creation skill unless the request also includes a failure diagnosis.
