...
- For Blender scene geometry, body or garment shaping, PMX/MMD conversion, part or weight transfer, component-safe integration, or exporter-preserving scene validation, also load `blender-xxmi-workflows`. Its inspected scene contract takes precedence over the generic and imported Blender examples here. Keep INI, runtime diagnosis, and DDS/render-path work with their existing specialized skills.
...
- Read [references/source-blender-and-export.md](references/source-blender-and-export.md) for basic manual Blender operations and legacy XXMI Tools UI context only when `blender-xxmi-workflows` does not already cover the scene task. Perform supported steps through an enabled Blender MCP; leave unsupported installation, add-on setup, and UI operations as user guidance.
