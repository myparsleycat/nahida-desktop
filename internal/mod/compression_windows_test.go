...
func TestApplyXpress4KCallsEveryEligibleFileOnceAndContinuesAfterFailure(t *testing.T) {
	...
	err := applyXpress4K(context.Background(), []string{root}, func(files int, _ int64) {
		...
		processed.Add(1)
	}, ignoreCompressionMutations, func(string, error) {
		fileErrors.Add(1)
	})
	if err != nil {
		t.Fatal(err)
...
func TestRestoreWOFInspectsEveryRegularFileAndDeletesOnlyFileProvider(t *testing.T) {
	root := t.TempDir()
	for _, name := range []string{"plain.bin", "xpress.bin", "wim.bin", "lzx.bin"} {
		writeCompressionTestFileAt(t, root, name, []byte("payload"))
	}
	...
	if err := restoreWOF(context.Background(), []string{root}, func(files int, _ int64) {
		...
		processed.Add(1)
	}, ignoreCompressionMutations, func(string, error) {}); err != nil {
		t.Fatal(err)
	}
	if inspected.Load() != 4 || total.Load() != 4 || processed.Load() != 4 || deleted.Load() != 2 {
