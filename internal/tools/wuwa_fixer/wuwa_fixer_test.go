...
func TestWuwaRunLetsFileDestinationReachExternalToolLog(t *testing.T) {
	...
	var logs []fixtool.FixToolLogEvent
	runner := fixtool.NewWithOptions(fixtool.Options{EventEmit: func(name string, data ...any) {
		if name == fixtool.LogEventName && len(data) == 1 {
			if event, ok := data[0].(fixtool.FixToolLogEvent); ok {
