...
// LogEventName is the renderer event carrying fix tool log lines.
const LogEventName = "ftm:log"
...
// Log streams a line to the fix tool log shown in the renderer.
func (t *Service) Log(message string, replaceLast bool) {
	t.emitEvent(LogEventName, FixToolLogEvent{Message: message, ReplaceLast: replaceLast})
}

func (t *Service) GetScripts(ctx context.Context) ([]db.ScriptBasicRow, error) {
...
func (t *Service) SaveScript(ctx context.Context, inputPath string) error {
	client, err := t.requireClient()
	if err != nil {
		return err
	}
...
func (t *Service) DeleteScript(ctx context.Context, scriptID string) error {
	client, err := t.requireClient()
	if err != nil {
		return err
	}
...
func (t *Service) GetPresets(ctx context.Context) ([]db.ScriptPresetWithScripts, error) {
	client, err := t.requireClient()
	if err != nil {
		return nil, err
	}
...
func (t *Service) IsPythonAvailable(ctx context.Context) bool {
	cmd := exec.CommandContext(ctx, "python", "--version")
	return cmd.Run() == nil
}

func (t *Service) CreatePreset(ctx context.Context, input CreateScriptPresetInput) error {
	client, err := t.requireClient()
	if err != nil {
		return err
	}
...
func (t *Service) DeletePreset(ctx context.Context, presetID string) error {
	client, err := t.requireClient()
	if err != nil {
		return err
	}
...
func (t *Service) CancelRun() bool {
	...
	t.runMu.Unlock()
	if run != nil {
		t.Log("Cancelled...", false)
		return true
	}
	return false
}

func (t *Service) SendInput(input string) bool {
	if t == nil {
		return false
	}
	t.runMu.Lock()
...
func (t *Service) RunScript(ctx context.Context, scriptID, destPath string) error {
	run, err := t.Begin(ctx)
	if err != nil {
...
func (t *Service) RunPreset(ctx context.Context, presetID, destPath string) error {
	run, err := t.Begin(ctx)
	if err != nil {
		return t.ReportRunError(err)
	}
	...
	t.Log("Starting Preset: "+preset.Name, false)
	for i, script := range scripts {
		if runCtx.Err() != nil {
			t.Log("Preset execution aborted by user.", false)
			return nil
		}
		if script == nil {
			t.Log(fmt.Sprintf("Script not found (ID: %s), skipping...", preset.Scripts[i].ScriptID), false)
			continue
		}
		_ = t.runScriptSafe(runCtx, run, script, destPath, nil)
	}
	if runCtx.Err() == nil {
		t.Log("Preset Completed", false)
	}
	return nil
}

// ReportRunError logs a failed run and reports it through the log stream instead
// of returning it, so the renderer shows one message per failure.
func (t *Service) ReportRunError(err error) error {
	t.logError(err, "FixTool")
	t.Log("Error: "+err.Error(), false)
	return nil
}
...
func (t *Service) runScriptSafe(
	...
	if script.IsSrcZstd {
		...
		if err != nil {
			t.Log(fmt.Sprintf("Failed %s: %s", script.Name, err), false)
			return false
		}
		data = decoded
	} else {
	...
	wrote = true
	t.Log("Running "+script.Name+"...", false)
	if err := run.Execute(scriptPath, script.Type, destPath, args); err != nil {
		if errors.Is(err, context.Canceled) {
			t.Log("Cancelled "+script.Name, false)
		} else {
			t.Log(fmt.Sprintf("Failed %s: %s", script.Name, err), false)
		}
		return false
	}
	t.Log("Completed "+script.Name, false)
	return true
}
