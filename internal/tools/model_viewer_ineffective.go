...
func (t *Tools) GetModelViewerIneffectiveValues(ctx context.Context, sessionID string, state map[string]any) (result []ModelViewerIneffectiveValue, err error) {
	...
	defer func() {
		if err != nil && ctx.Err() == nil {
			...
			err = infra.ReportError(t.log, err, "Tools.GetModelViewerIneffectiveValues", infra.Diagnostic{Operation: "evaluate-toggle-choices", Stage: "evaluate", Fields: fields})
		}
	}()
...
func (e *modelViewerEvaluator) evaluate(ctx context.Context, state map[string]any) ([]ModelViewerIneffectiveValue, error) {
	...
	for _, variable := range e.payload.Variables {
		...
		for _, other := range e.blocking[variable.ID] {
			if value, ok := viewerEvalLookup(resolved, other.ID); ok {
				blocking = append(blocking, ModelViewerBlockingVariable{ID: other.ID, Label: other.Label, Value: modelViewerString(value)})
			}
		}
		for _, entry := range variable.Values {
			...
			result = append(result, ModelViewerIneffectiveValue{VariableID: variable.ID, Value: modelViewerString(entry.Value), BlockingVars: blocking, Suggestions: suggestions})
		}
	}
	return result, ctx.Err()
}

func (e *modelViewerEvaluator) suggestions(ctx context.Context, tested ModelViewerVariable, value string, state map[string]any, baseline viewerEvalState) ([]ModelViewerResolutionSuggestion, error) {
	...
	for _, blocking := range e.blocking[tested.ID] {
		...
		for _, entry := range blocking.Values {
			...
			if !viewerEvalStatesDiffer(evaluateViewerTransport(e.payload, alternative), testEval) || !viewerEvalStatesDiffer(baseline, testEval) {
				continue
			}
			...
			for _, variable := range e.payload.Variables {
				...
				if from != to {
					changes = append(changes, ModelViewerResolutionChange{VarID: variable.ID, VarLabel: variable.Label, FromValue: from, ToValue: to})
				}
			}
			result = append(result, ModelViewerResolutionSuggestion{Display: fmt.Sprintf("%s: %s → %s", blocking.Label, modelViewerString(current), modelViewerString(entry.Value)), Changes: changes})
		}
	}
	return result, nil
}
