...
func (t *Service) ZZMIFixerActivateLatestRules(ctx context.Context) (ZZMIFixerRuleStatus, error) {
...
func (t *Service) ZZMIFixerRun(ctx context.Context, input ZZMIFixerRunInput) (ZZMIFixerRunResult, error) {
	...
	result, err := zzmiengine.Run(
		...
		input.Tool,
		pack,
		func(message string) { t.runner.Log(message, false) },
	)
	if err != nil {
	...
	if len(result.Changes) == 0 {
		t.runner.Log("No changes were required.", false)
		return output, nil
	}
	stage = "commit"
	session, err := t.zzmiCommit(run.Context(), target, input.Tool, *pack, result.Changes)
	if err != nil {
...
func (t *Service) ZZMIFixerListBackups(ctx context.Context, targetPath string) ([]ZZMIBackupSession, error) {
	target, _, err := t.zzmiRequireTarget(ctx, targetPath)
	if err != nil {
		return nil, err
	}
...
func (t *Service) ZZMIFixerGetBackup(ctx context.Context, targetPath, sessionID string) (ZZMIBackupSession, error) {
	target, _, err := t.zzmiRequireTarget(ctx, targetPath)
	if err != nil {
...
func (t *Service) ZZMIFixerRestore(ctx context.Context, input ZZMIFixerRestoreInput) (ZZMIFixerRestoreResult, error) {
	target, _, err := t.zzmiRequireTarget(ctx, input.Path)
	if err != nil {
...
func (t *Service) ZZMIFixerDeleteBackup(ctx context.Context, input ZZMIFixerDeleteBackupInput) error {
	target, _, err := t.zzmiRequireTarget(ctx, input.Path)
	if err != nil {
		return err
	}
...
func (t *Service) ZZMIFixerDeleteAllBackups(ctx context.Context, targetPath string) error {
	target, _, err := t.zzmiRequireTarget(ctx, targetPath)
	if err != nil {
		return err
	}
...
func (t *Service) zzmiRequireTarget(ctx context.Context, targetPath string) (string, string, error) {
...
func (t *Service) zzmiStorePack(data []byte, pack zzmiengine.RulePack) error {
	dir, err := t.appDataPath(filepath.Join(appdata.ToolsDir, zzmiFixerDirName))
	if err != nil {
		return err
	}
...
func (t *Service) zzmiGetCachedLatestRelease(ctx context.Context) *zzmiLatestRelease {
...
func (t *Service) zzmiRememberLatest(ctx context.Context, latest *zzmiLatestRelease) error {
	stored := *latest
...
func (t *Service) zzmiCheckLatest(ctx context.Context, force bool) (*zzmiLatestRelease, bool, error) {
...
//wails:ignore
func (t *Service) CleanupZZMIAbandonedStaging(ctx context.Context) error {
