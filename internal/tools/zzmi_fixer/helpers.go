...
// file, so an interrupted restore never leaves a partial file behind.
func copyRegularFile(source, target string) (returnErr error) {
	...
	if err != nil || !info.Mode().IsRegular() {
		return infra.WithCause(errors.New("restore source is not a regular file"), err)
	}
	if err := os.MkdirAll(filepath.Dir(target), 0o755); err != nil {
		return err
	}
	temp, err := os.CreateTemp(filepath.Dir(target), ".zzmi-restore-*")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
