...
func (d *Drive) ListLinkChildren(
	ctx context.Context,
	params ListLinkChildrenParams,
) (result ListChildrenResult, err error) {
...
func (d *Drive) ListModChildren(
	ctx context.Context,
	params ListModChildrenParams,
) (result ListChildrenResult, err error) {
...
func (d *Drive) requestSharedLinkAccess(ctx context.Context, linkID, password string) (SharedLinkAccess, error) {
	...
	if edenErr != nil {
		return SharedLinkAccess{}, remapLinkAccessError(
			CreateDriveAPIError(edenErr.asAny(), "shared link access", edenErr.Status),
		)
	}
...
func (d *Drive) requestModOverview(ctx context.Context, modID, password string) (ModAccess, error) {
	...
	data, headers, edenErr, err := d.doJSONHeaders(
		ctx,
		http.MethodGet,
		"/akasha/mod/"+url.PathEscape(modID),
		query,
		nil,
		nil,
	)
	...
	if edenErr != nil {
		return ModAccess{}, remapModOverviewError(
			CreateDriveAPIError(edenErr.asAny(), "mod overview", edenErr.Status),
			hasPassword,
		)
	}
...
func (d *Drive) requestModContent(ctx context.Context, itemID, modToken, modSig string) (ListChildrenResult, error) {
	...
	data, _, edenErr, err := d.doJSONHeaders(
		ctx,
		http.MethodGet,
		"/akasha/mod/item/"+url.PathEscape(itemID),
		nil,
		extra,
		nil,
	)
...
func remapModOverviewError(err *DriveAPIError, hasPassword bool) error {
	...
	code := strings.ToLower(err.Code)
	message := strings.ToLower(err.Message)
	if code == "drive_mod_password_required" || strings.Contains(message, "password required") ||
		strings.Contains(message, "missing_password") {
		return newDriveAPIError(codeModPasswordRequired, msgModPasswordRequired, err.Status, err)
	}
	if code == "drive_mod_invalid_password" || strings.Contains(message, "invalid password") ||
		strings.Contains(message, "invalid_password") {
...
func (d *Drive) doJSONHeaders(
	ctx context.Context,
	method, path string,
	query url.Values,
	extra http.Header,
	body any,
) (any, http.Header, *edenError, error) {
...
func (d *Drive) doRequest(
	ctx context.Context,
	method, path string,
	query url.Values,
	extra http.Header,
	body any,
) (*http.Response, any, *edenError, error) {
	if d == nil || d.http == nil {
