...
export function useGameMutations() {
    ...
    const deleteGameMutation = useMutation({
        mutationFn: (game: string) => Mod.RemoveGame(game),
        onSuccess: async (_, deletedGame) => {
            ...
            if (selectedGame === deletedGame) {
                ...
                await Mod.SetLastGame(nextSelectedGame);
            }
    ...
    const updateGameMutation = useMutation({
        ...
        onSuccess: async (_, variables) => {
            ...
            if (selectedGame === variables.game) {
                ...
                await Mod.WatchGame(variables.game);
            }
    ...
    const reorderGamesMutation = useMutation({
        mutationFn: (games: string[]) => Mod.ReorderGames(games),
    ...
    const setNteLauncherPathMutation = useMutation({
        mutationFn: ({ game, launcherPath }: { game: string; launcherPath: string }) =>
            Mod.SetNteLauncherPath(game, launcherPath),
        onSuccess: () => {
            void queryClient.invalidateQueries({ queryKey: ["games"] });
        },
        onError: (error) => {
...
export function useModMutations() {
    ...
    const updateLocalGroupCache = (refreshedGroup: FolderGroup) => {
        ...
        queryClient.setQueryData(["modGroupLight", refreshedGroup.path], refreshedGroup);
    };
    ...
    const toggleModMutation = useMutation({
        mutationFn: async (mod: ModInfo) => {
            try {
                await Mod.Toggle(mod.path);
                const currentGroupPath = selectedGroup?.path;
                if (currentGroupPath) {
                    const refreshedGroup = (await Mod.GetMods(currentGroupPath)) as FolderGroup;
    ...
    const exclusiveToggleModMutation = useMutation({
        mutationFn: async (mod: ModInfo) => {
            try {
                await Mod.ExclusiveToggle(mod.path);
                const currentGroupPath = selectedGroup?.path;
                if (currentGroupPath) {
                    const refreshedGroup = (await Mod.GetMods(currentGroupPath)) as FolderGroup;
                    return refreshedGroup;
    ...
    const renameModMutation = useMutation({
        mutationFn: async ({
            ...
            try {
                await Mod.Rename(mod.path, newName);
...
export function usePresetMutations() {
    ...
    const createPresetMutation = useMutation({
        mutationFn: ({
            ...
            return Mod.CreatePreset(selectedGame, name, description, resolveConflicts);
        },
        onSuccess: () => {
    ...
    const getPresetCreateConflicts = async (): Promise<PresetCreateConflict[]> => {
        ...
        return ((await Mod.GetPresetCreateConflicts(selectedGame)) ?? []) as PresetCreateConflict[];
    };

    const applyPresetMutation = useMutation({
        mutationFn: (presetId: string) => Mod.ApplyPreset(presetId),
        onSuccess: (result) => {
            void queryClient.invalidateQueries({ queryKey: ["modGroup"] });
            void queryClient.invalidateQueries({ queryKey: ["characters", selectedGame] });
    ...
    const deletePresetMutation = useMutation({
        mutationFn: (presetId: string) => Mod.DeletePreset(presetId),
        onSuccess: () => {
            void queryClient.invalidateQueries({ queryKey: ["presets", selectedGame] });
            setSelectedPreset(null);
            setIsSelectedPresetDialogOpen(false);
