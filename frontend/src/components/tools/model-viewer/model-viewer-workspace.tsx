...
export function ModelViewerWorkspace({
  ...
  onPreviewSaved?: (path: string) => void | Promise<void>;
  ...
  const captureAndSavePreview = async () => {
    ...
    const savedPath = await Mod.PastePreview(
      source.modPath,
      dataUrl,
      "base64",
      existingPreviewPath ?? null,
    );
    await onPreviewSaved?.(savedPath);
  };
                                          ...
                                          <div className="w-0 overflow-hidden transition-all group-hover:w-8 focus-within:w-8">
                                            <Tooltip>
                                              ...
                                              <TooltipContent>
                                                <div className="space-y-1 text-left">
                                                  <div>
                                                    {t("page.tools.model_viewer.ineffective_apply")}
                                                  </div>
                                                  <ul className="ml-3 list-disc">
