...
import { Dialog as NativeDialog, Shell } from "@bindings/platform";
...
export function TextureResizerWorkspace({
  ...
  const loadTextures = async (
    ...
    try {
      const nextTextures = await resolveIfCurrent(requestId, loadRequestIdRef, () =>
        mode === "mod"
          ? Tools.ListTextureMod(normalizedTargetPath, nextSettings)
          : Tools.ListTextureFolder(normalizedTargetPath, nextSettings),
      );
      if (nextTextures === undefined) {
        return;
      }
      setTextures((nextTextures ?? []) as TextureResizeListItem[]);
      setLoadedTargetPath(normalizedTargetPath);
      const available = new Set(
        (nextTextures ?? [])
          .filter((texture) => texture.canProcess)
          .map((texture) => texture.filePath),
      );
      setSelectedPaths(
  ...
  const browseTargetPath = async () => {
    const selected = await NativeDialog.ShowOpenDialog({
      title: "",
      defaultPath: "",
      filters: [],
  ...
  useEffect(() => {
    return Events.On("tools:textureUpscaleProgress", (event) => {
      setUpscaleProgress(event.data);
    });
  }, []);

  useEffect(() => {
    const requestId = ++loadRequestIdRef.current;
    Tools.GetTextureResizeSettings()
      .then((nextSettings) => {
        if (!isCurrentRequest(requestId, loadRequestIdRef)) {
          return;
        }
        settingsForm.reset(nextSettings as TextureResizeSettings);
        if (mode === "mod" && fixedTargetPath) {
          void loadTextures(fixedTargetPath, nextSettings as TextureResizeSettings, requestId);
        }
      })
      .catch((error) => {
        if (!isCurrentRequest(requestId, loadRequestIdRef)) {
  ...
  const processSelectedTextures = async (dialogSettings: TextureResizeSettings) => {
    ...
    for (const [index, texture] of selected.entries()) {
      ...
      setUpscaleProgress(null);
      try {
        const nextResult = await Tools.ResizeTextureFile({
          filePath: texture.filePath,
          settings: nextSettings,
        });
        const fileResult = (nextResult.files ?? [])[0] as TextureResizeFileResult | undefined;
      ...
      <td className="w-full max-w-0 p-2 pl-3 text-left align-middle whitespace-nowrap">
        <div
          ...
          onClick={(event) => {
            ...
            void Shell.OpenExternal(texture.filePath);
          }}
        >
          {texture.fileName}
        </div>
