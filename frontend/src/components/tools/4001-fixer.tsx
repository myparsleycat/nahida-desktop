...
export default function FourThousandOneFixer() {
  ...
  useEffect(() => {
    ...
    void Tools.FourThousandOneFixerGetState().then((state) => {
      ...
      setActiveTask(state.activeTask as FixerTask | null);
      setProgress(state.progress || "");
      setErrorMessage(state.errorMessage || "");
    });

    void XXMI.GetXXMIData().then((data) => {
      const gimiImporters =
    ...
    const off = Events.On("tools:4001FixerProgress", (event) => {
      const payload = event.data as { task?: string; code?: string; errorMessage?: string };
      setProgress(payload.code ?? "");
      setErrorMessage(payload.errorMessage || "");

      if (payload.task) {
        setActiveTask(payload.task as FixerTask);
      }

      if (isTerminalFixerProgressCode(payload.code ?? "")) {
        setActiveTask(null);
      }
    });
  ...
  useEffect(() => {
    ...
    void Tools.FourThousandOneFixerGetProviderReleases(requestedProvider)
      .then((v) => {
  ...
  useEffect(() => {
    ...
    void Tools.FourThousandOneFixerGetDiversificationState({ importerPath: selectedImporter }).then(
      (state) => {
        if (cancelled) return;
    ...
    void Tools.FourThousandOneFixerCheckImporterWriteAccess({ importerPath: selectedImporter })
      .then((access) => {
        if (cancelled) return;
  ...
  const handleBuild = async () => {
    ...
    try {
      const result = await Tools.FourThousandOneFixerBuildDll({
        provider,
        version,
  ...
  const handleDiversify = async () => {
    ...
    try {
      const result = await Tools.FourThousandOneFixerDiversifyDllPadding({
        importerKey: selectedImporterKey,
        importerPath: selectedImporter,
      });
  ...
  const handleRestore = async () => {
    ...
    try {
      const result = await Tools.FourThousandOneFixerRestoreDiversifiedDll({
        importerPath: selectedImporter,
      });
      if (result?.success === false) {
        setErrorMessage(result && typeof result === "object" ? result.errorMessage || "" : "");
