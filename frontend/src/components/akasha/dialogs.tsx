...
export function RenameDialog() {
  ...
  const mutation = useMutation({
    ...
    mutationFn: async ({ item, rename }: { item: Content; rename: string }) => {
      const data = await Drive.Rename(item.id, rename);
      return data;
    },
    onSuccess: async () => {
...
export function DeleteItemsDialog() {
  ...
  const deleteMutation = useMutation({
    ...
    mutationFn: async (ids: string[]) => {
      return Drive.DeleteItems(ids, "delete");
    },
  });
  ...
  const handleDelete = async () => {
    ...
    await deleteMutation
      ...
      .then(async (outcome) => {
        const acceptedIds = outcome.acceptedIds ?? [];
        const requestedIds = outcome.requestedIds ?? [];
