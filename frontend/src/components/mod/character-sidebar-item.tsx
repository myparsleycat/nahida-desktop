...
export const CharacterSidebarItem = memo(function CharacterSidebarItem({
  ...
  useWindowFileDrop(({ paths, target }) => {
    if (target.attributes[FILE_DROP_GROUP_PATH_ATTRIBUTE] !== group.path) {
      return;
    }
    setDragOverIfChanged(false);
    onDrop?.(group, paths);
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger
        render={
          <div
            ...
            data-file-drop-group-path={group.path}
      ...
      <ContextMenuContent className="w-56">
        ...
        <ContextMenuItem onClick={() => void Shell.OpenPath(group.path)}>
