...
import {
    ...
    extractPositionSectionMatchLines,
    hasMasterSwapRef,
    isBackupIniName,
...
export async function writeNamespaceMerge(options: {
    ...
    const positionMatchLines = representative
        ? extractPositionSectionMatchLines(representativeText)
        : [];
...
function precedesMasterSwapIf(
    lines: ReturnType<typeof parseIniText>["sections"][number]["lines"],
    index: number,
) {
    const upcoming = lines.slice(index + 1).find((entry) => {
        if (entry.raw.trim() === "") return false;
        return !(entry.kind === "kv" && isOverrideMatchKey(entry.key));
    });
    return upcoming !== undefined && MASTER_SWAP_IF_RE.test(upcoming.raw.trim());
}
...
function buildWrappedBlock(block: string[], name: string, baseIndex: number) {
    ...
    for (const raw of block) {
        ...
        if (line.kind === "kv" && line.key.toLowerCase() === "match_priority") continue;
        if (line.kind === "kv" && isOverrideMatchKey(line.key)) {
            matchLines.push(raw.trimStart());
            continue;
        }
        commandLines.push(raw);
    }
    return [
        `match_priority = ${baseIndex}`,
        ...matchLines,
        `if $\\${name}\\Master\\swapvar==${baseIndex}`,
        ...closeWrap(commandLines),
    ];
}
