...
import { OverlayProcess } from "./progress";
import { subscribe } from "@main/lib/process-monitor";

export class OverlayManager {
    private readonly map: Map<number, OverlayProcess> = new Map();
    private keybindKeys: string[] = ["Control", "Shift", "Space"];
    private EMITTERY: any | undefined;
    ...
    async startMonitoring() {
        if (!this.EMITTERY) {
            this.EMITTERY = await subscribe({
                creation: true,
                deletion: true,
            });
        }

        this.EMITTERY.on("creation", async ([processName, pid]: [string, number | string]) => {
            if (
                processName.toLowerCase() === "genshinimpact.exe" ||
                processName.toLowerCase() === "yuanshen.exe"
            ) {
                await this.runOverlay(Number(pid));
            }
        });
    }
