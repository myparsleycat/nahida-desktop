...
export function registerModBisectHandlers(d: NahidaDesktop) {
    rh("tools:bisectStart", (game: string, excludePaths: string[] = []) =>
        d.service.modTools.modBisect.start(game, excludePaths),
    );
    rh("tools:bisectValidateExcludePath", (game: string, inputPath: string) =>
        d.service.modTools.modBisect.validateExcludePath(game, inputPath),
    );
