...
class ModsService {
  ...
  folder = {
    ...
    dir: {
      ...
      disableAll: async (path: string) => {
        try {
          const children = await fss.readDirectory(path);
          ...
          Toast.success(`${(basename(path))}의 모든 모드가 비활성화 되었습니다`);
          return true;
        } catch (e: any) {
          Toast.error("전체 비활성화 중 오류 발생", {
            description: e.message
          });
          return false;
        }
      ...
      enableAll: async (path: string) => {
        try {
          ...
          Toast.success(`${(basename(path))}의 모든 모드가 활성화 되었습니다`);
          return true;
        } catch (e: any) {
          Toast.error("전체 활성화 중 오류 발생", {
            description: e.message
          });
          return false;
        }
  ...
  ini = {
    ...
    update: async (path: string, section: string, key: 'key' | 'back', value: string) => {
      try {
        ...
        await fss.writeFile(path, updatedContent, { encoding: 'utf8' });
        Toast.success(`토글 키가 변경되었습니다`);
        return true;
      } catch (err: any) {
        console.error(`Error updating INI file: ${err.message}`);
        Toast.error('토글 수정중 오류 발생', {
  ...
  intx = {
    drop: async (data: string[]) => {
      if (!this.currentCharPath) {
        Toast.warning('모드가 저장될 폴더를 선택해주세요');
        return false;
      }
      ...
      for (const item of data) {
        if (Validator.url(item)) {
          ...
          try {
            const stats = await fss.getStat(item);
            ...
            Toast.error(`경로를 확인할 수 없습니다: ${item}`, err.message);
            return false;
          }
        }
      }
      ...
      if (urls.length === 0 && folders.length === 0 && archives.length === 0) {
        Toast.warning('처리할 수 있는 URL 또는 파일 데이터가 없습니다');
        return false;
      }
      ...
      try {
        ...
        for (const folder of folders) {
          await fss.copy(folder, this.currentCharPath);
        }

        Toast.success('작업이 완료되었습니다');
        return true;
      } catch (e: any) {
        Toast.error('작업중 오류 발생', {
          description: e.message
        });
        return false;
      }
  ...
  async getDirectChildren(path: string, recursive: number) {
    const children = await fss.readDirectory(path, { recursive });
