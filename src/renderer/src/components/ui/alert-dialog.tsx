    ...
    <AlertDialogPrimitive.Root
      ...
      onOpenChange={(nextOpen, eventDetails) => {
        if (
          ...
          isTitlebarTarget(eventDetails.event.target)
        ) {
          eventDetails.cancel();
          return;
        }
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        data-size={size}
        className={cn(
          "group/alert-dialog-content fixed top-1/2 left-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-popover/50 p-4 text-popover-foreground ring-1 ring-foreground/10 backdrop-blur-lg duration-100 outline-none data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-sm",
          className,
        )}
        finalFocus={false}
        {...props}
